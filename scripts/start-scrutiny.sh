...
# .ai/config.json から tmuxSession（ベース名）と agentCommand を取得（Node で読む）
get_config() {
  node -e "
    ...
    try {
      ...
      console.log(JSON.stringify({ tmuxSession: 'scrutiny', agentCommand: '' }));
    }
...
# セッション agent: agentCommand（1 ペイン）。Submit はここに送られる。
tmux new-session -d -s "$AGENT_SESSION" -c "$PROJECT_ROOT"
sleep 0.5
if [ -n "$AGENT_CMD" ]; then
